...
export const createGeminiProvider = ({ apiKey, model, fetchImpl = fetch }) => ({
  async generateStructured({ system, user, schema, signal }) {
    if (!apiKey || !model) throw new AIConfigurationError('Thiếu cấu hình Gemini.');
    const response = await fetchImpl(`https://generativelanguage.googleapis.com/v1beta/models/${encodeURIComponent(model)}:generateContent`, {
      method: 'POST', signal, headers: { 'x-goog-api-key': apiKey, 'Content-Type': 'application/json' },
    ...
    return { value: JSON.parse(text), usage: { input_tokens: data.usageMetadata?.promptTokenCount, output_tokens: data.usageMetadata?.candidatesTokenCount }, model };
  },
});
