...
// Chat agent
router.post('/api/python-assistant/chat', authenticate, [
...
// Streaming chat (SSE)
router.post('/api/python-assistant/chat/stream', authenticate, [
  body('question').trim().isLength({ min: 1, max: 4000 }).withMessage('Câu hỏi phải từ 1-4000 ký tự'),
  body('nhanh').isIn(NHANH_VALUES).withMessage('Nhánh không hợp lệ'),
  body('lichSu').optional().isArray().withMessage('Lịch sử phải là mảng'),
  validate,
...
// Tìm kiếm tài liệu
router.post('/api/python-assistant/search', authenticate, [
...
router.post('/api/python-assistant/learning-path/progress', authenticate, requireRole('student'), [
...
router.get('/api/python-assistant/documents', authenticate, requireRole('teacher'), [
...
router.post('/api/python-assistant/documents/upload', authenticate, requireRole('teacher'), [
...
router.get('/api/python-assistant/documents/:id', authenticate, [
  param('id').isUUID().withMessage('ID tài liệu không hợp lệ'),
  validate,
], pythonAssistantController.getDocumentDetail);

router.get('/api/python-assistant/documents/:id/images', authenticate, [
  param('id').isUUID().withMessage('ID tài liệu không hợp lệ'),
  validate,
], pythonAssistantController.getDocumentImages);

// Worker queue processing (có thể gọi từ cron)
...
router.get('/api/python-assistant/exercises/pending', authenticate, requireRole('teacher'), [
  query('nhanh').optional().isIn(NHANH_VALUES),
  query('muc_do').optional().isIn(['CB', 'TB', 'NC', 'HSG']),
  validate,
], exerciseReviewController.listPendingExercises);

router.post('/api/python-assistant/exercises/:id/review', authenticate, requireRole('teacher'), [
...
// Đề thi
router.get('/api/python-assistant/exams/pending', authenticate, requireRole('teacher'), exerciseReviewController.listDeThiPending);

router.post('/api/python-assistant/exams/:id/review', authenticate, requireRole('teacher'), [
  param('id').isUUID().withMessage('ID đề thi không hợp lệ'),
  body('decision').isIn(['da_duyet', 'tu_choi']).withMessage('Quyết định không hợp lệ'),
  validate,
], exerciseReviewController.reviewDeThi);

// Tra cứu web (tuỳ chọn, qua Edge Function)
router.post('/api/python-assistant/web-search', authenticate, [
