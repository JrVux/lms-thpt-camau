...
export const createStudentAssignmentService = (db) => {
  ...
  const getAuthorizedDelivery = async (userId, deliveryId) => {
    ...
    if (delivery.assignments?.submission_type === 'essay' && submissions?.length) {
      const { data: reports, error: reportsError } = await db.from('essay_grading_reports').select('*, essay_grading_jobs(model_answer_snapshot,rubric_snapshot,grading_method)').in('submission_id', submissions.map((item) => item.id)).not('published_at', 'is', null).order('published_at', { ascending: false });
  ...
  return {
    async listMine({ userId, status }) {
      ...
      if (studentSubmissionIds.length) {
        const { data: reports, error: reportsError } = await db.from('essay_grading_reports').select('*, essay_grading_jobs(model_answer_snapshot,rubric_snapshot,grading_method)').in('submission_id', studentSubmissionIds).not('published_at', 'is', null).order('published_at', { ascending: false });
        throwDbError(reportsError);
